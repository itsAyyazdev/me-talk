...
class ShopDetailsRepository {
  static Future<ShopDetailsBeansData> shopDetailsApiCall(
    ...
    try {
      ApiResponse apiResponse = await restClientNew.post(
          ...
          body: {"category_profile_id": categoryProfileId},
          header: {"Authorization": "Bearer $accessToken"});
      if (apiResponse.statusCode == 200) {
        LoadingWidget.endLoadingWidget(context);
        return ShopDetailsBeansData.fromJson(apiResponse.data);
      } else {
        LoadingWidget.endLoadingWidget(context);
  ...
  static Future<bool> updateShopDetailsApiCall(
    ...
    FormData formData = new FormData.fromMap({
      ...
      "specialty_of": specialOf ?? null,
      "slider_img": mainPhotoListPic != null ? uploadImages : null,
      "del_slider": deleteIds ?? null
    });
    try {
      ApiResponse apiResponse = await restClientNew.post(
          context, Api.UpdateCategoryDetails,
  ...
  static Future<bool> followBusinessApiCall(
      BuildContext context, int categoryProfileId) async {
    String accessToken = await SharePreferencesHelper.getInstant()
        .getString(SharePreferencesHelper.Access_Token);
    try {
      ApiResponse apiResponse = await restClientNew.post(
          context, Api.FollowBusiness,
          body: {"category_profile_id": categoryProfileId},
          header: {"Authorization": "Bearer $accessToken"});
      if (apiResponse.statusCode == 200) {
        return true;
    ...
    static Future<bool> unFollowBusinessApiCall(
      BuildContext context, int categoryProfileId) async {
    String accessToken = await SharePreferencesHelper.getInstant()
        .getString(SharePreferencesHelper.Access_Token);
    try {
      ApiResponse apiResponse = await restClientNew.post(
          context, Api.UnFollowBusiness,
          body: {"category_profile_id": categoryProfileId},
          header: {"Authorization": "Bearer $accessToken"});
      if (apiResponse.statusCode == 200) {
        return true;
